...
if __name__ == "__main__":
    ...
    upload_parser.add_argument("-p", "--proxy", default="", help="Proxy to use for upload (format: ip:port:username:password)")
    ...
    if args.subcommand == "login":
        ...
        # Parse proxy format if provided (ip:port:username:password)
        proxy_url = None
        if args.proxy:
            try:
                ...
                if len(parts) == 4:
                    ...
                    print(f"[-] Invalid proxy format. Expected: ip:port:username:password, got: {args.proxy}")
                    sys.exit(1)
            except Exception as e:
                print(f"[-] Error parsing proxy: {e}")
                sys.exit(1)
        
        tiktok.upload_video(args.users, args.video, args.title, args.schedule, args.comment, args.duet, args.stitch, args.visibility, args.brandorganic, args.brandcontent, args.ailabel, proxy_url)
